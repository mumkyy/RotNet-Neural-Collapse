...
class NetworkInNetwork(nn.Module):
    def __init__(self, opt):
        ...
        for s in range(3, num_stages):
            blocks[s].add_module(f'Block{s+1}_ConvB1', BasicBlock(nChannels, nChannels, 3))
            blocks[s].add_module(f'Block{s+1}_ConvB2', BasicBlock(nChannels, nChannels, 1))
            blocks[s].add_module(f'Block{s+1}_ConvB3', BasicBlock(nChannels, nChannels, 1))
